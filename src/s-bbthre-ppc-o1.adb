...
package body System.BB.Threads is
   ...
   --  Start_Time : constant Peripherals.Timer_Interval := 500;
   ...
   begin
      ...
      --  Queues.Priorities_Threads_Mapping (Main_Priority)
      --  := Environment_Thread;
      ...
      --  The idle thread has priority level 0, we have to set the first
      --  bit of the first child bitmask (and so the first bit of the
      --  root bitmask)

      Queues.Ready_Root_Bitmask       := 2#00000000000000000000000010000000#;
      Queues.Ready_Child_Bitmasks (0) := 2#10000000000000000000000000000000#;
      ...
      --  Queues.Insert (Environment_Thread);
      ...
      --  Init the timer with the start time interval
      Peripherals.Set_Alarm (10 * (74 * (10**3)));
...
--        Put_Line ("Setting priority of running thread from " &
--                    System.Any_Priority'Image
--                    (Queues.Running_Thread.Active_Priority) &
--                    " to " & System.Any_Priority'Image (Priority));
      Queues.Change_Priority (Queues.Running_Thread, Priority);
--        Put_Line ("Priority is now set to " &
--                    System.Any_Priority'Image
--                    (Queues.Running_Thread.Active_Priority));
...
--        Protection.Leave_Kernel;
   ...
   is
      use Threads.Queues;
      package Int_Arg is new System.Address_To_Access_Conversions (Integer);
      a1_ptr : constant Int_Arg.Object_Pointer :=
        Int_Arg.To_Pointer (Task_Args);
      a2_ptr : constant Int_Arg.Object_Pointer :=
        Int_Arg.To_Pointer (System.Storage_Elements."+" (Task_Args, 4));
      a3_ptr : constant Int_Arg.Object_Pointer :=
        Int_Arg.To_Pointer (System.Storage_Elements."+" (Task_Args, 8));
      a4_ptr : constant Int_Arg.Object_Pointer :=
        Int_Arg.To_Pointer (System.Storage_Elements."+" (Task_Args, 12));
      a5_ptr : constant Int_Arg.Object_Pointer :=
        Int_Arg.To_Pointer (System.Storage_Elements."+" (Task_Args, 16));
--        before : Peripherals.Timer_Interval;
--        after  : Peripherals.Timer_Interval;
   begin
      Put_Line ("Thread_Create procedure");
--        Put_Line ("Thcreate: " & To_Integer (Id)'Img  & " Code: " &
--                  Code'Img & " - Arg: " & Arg'Img);

--        Put_Line ("Params for task at priority " &
--                    System.Any_Priority'Image (Priority) &" are: NPR = "  &
--                    Integer'Image (a1_ptr.all) & ", CS = " &
--                    Integer'Image (a2_ptr.all) & ", Phase = " &
--                    Integer'Image (a3_ptr.all) & ", WCET = " &
--                    Integer'Image (a4_ptr.all) & ", Period = " &
--                    Integer'Image (a5_ptr.all)
--                 );
      Id.NPR_Length  := a1_ptr.all;
      Id.CS_Length   := a2_ptr.all;
      Threads.Queues.Tasks_Phases (Priority_Range (Priority)) := a3_ptr.all;
      Id.WCET_Budget := a4_ptr.all;
      Id.Remaining_WCET := Id.WCET_Budget;
      Threads.Queues.Tasks_Periods (Priority_Range (Priority)) := a5_ptr.all;

      if Priority_Range (Priority) = Threads.Queues.Priority_Range'Last then
         Put ("Phases: ");
         for i in Priority_Range loop
            Put (Integer'Image (Tasks_Phases (i)) & " ");
         end loop;
         New_Line;
         Put ("Periods: ");
         for i in Priority_Range loop
            Put (Integer'Image (Tasks_Periods (i)) & " ");
         end loop;
         New_Line;
      end if;

--        Id.Nr_Of_Activations :=
--          Threads.Queues.Hyperperiod /
--            Threads.Queues.Tasks_Periods (Id.Base_Priority);

--        before := Peripherals.Read_Clock;
--        for i in 0 .. 3 loop
--           Asm ("nop", Volatile => True);
--        end loop;
--
--        after := Peripherals.Read_Clock;
--        Put_Line ("***** TIME ELAPSED:" &
--                    Peripherals.Timer_Interval'Image (after - before));

--        Put_Line ("AB: priority " & System.Any_Priority'Image (Priority) &
--                  ", stack address " & System.Address'Image (Stack_Address) &
--                    ", stack size " &
--                    System.Parameters.Size_Type'Image (Stack_Size));
      ...
      --  Init the priority level of the thread in the priorities-threads
      --  mapping

      Queues.Priorities_Threads_Mapping (Id.Active_Priority) := Id;
      ...
      Threads.Queues.Tasks_Ids
        (Threads.Queues.Priority_Range (Priority)) := Id;
      --  Add the initial alarm (the start time alarm)
--        Threads.Queues.Add_Initial_Alarm (Id, Priority);

      --  If the thread is periodic, create all the (delay until) alarms
--        if Threads.Queues.Tasks_Periods
--          (Priority_Range (Id.Base_Priority)) /= 0 then
--           Threads.Queues.Add_Alarms (Id,
--                                      Threads.Queues.Tasks_Periods
--                                        (Priority_Range (Id.Base_Priority)),
--                    Priority);
--        end if;

      --  TODO: move this to a better place
      if Priority_Range (Priority) = Threads.Queues.Priority_Range'Last then
         Threads.Queues.Create_Alarms;
      end if;

--        Threads.Queues.Print_List_From (0);
      --  Threads.Queues.First_Alarm

      --  Threads.Queues.Print_List_From (Threads.Queues.Current_Alarm);
...
--        Id.Remaining_WCET      := 6;
--        Id.CS_Length           := 1;
      --  Id.Extraction_Pending  := False;

      Protection.Leave_Kernel;
      --  Put_Line (" <- ThCreate");
      Put_Line ("End Thread_Create procedure for thread at priority " &
...
--        System.BB.Serial_Output.Put_Line ("");
   ...
   begin
      ...
      --  System.BB.Serial_Output.Put ("BEGIN Timer_Execution_Time_Adjust: ");
      if Next.Active_Priority > 0 then
         --  System.BB.Serial_Output.Put ("A ");
         ...
         if Running.Time_Init_Execution /= System.BB.Time.Time'Last then
            ...
            --  System.BB.Serial_Output.Put ("F ");
            Next.Is_Timer_Alarm := False;
            System.BB.Threads.Queues.TM_Alarm := System.BB.Time.Time'Last;
            if Next.Time_Remaining /= System.BB.Time.Time'Last then

               --  System.BB.Serial_Output.Put ("G ");
               --  Insert alarm if needed
               System.BB.Threads.Queues.Insert_TM_Alarm
                 (Now + Next.Time_Remaining, Is_First_Alarm);
               Insert_Alarm_If_Needed (Is_First_Alarm,
         ...
         if Running.Time_Remaining /= System.BB.Time.Time'Last then
            ...
            if Running.Time_Remaining < (Elapsed_Time - Running.Time_Diff) then
               ...
               --  System.BB.Serial_Output.Put ("I ");
               null;

            else
               --  System.BB.Serial_Output.Put ("J ");
