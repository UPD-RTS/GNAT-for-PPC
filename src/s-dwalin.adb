...
package body System.Dwarf_Lines is
   ...
   begin
      Tell (C.Obj, Saved_Off);
      Seek (C.Obj, C.Prologue.Includes_Offset);

      J := 0;
      loop
         J := J + 1;
         Tell (C.Obj, Off);
         Read_C_String (C.Obj, Buf);

         if Strlen (Buf) = 0 then
            Seek (C.Obj, Saved_Off);
            return 0;
         end if;
   ...
   begin
      Tell (C.Obj, Saved_Off);
      Seek (C.Obj, C.Prologue.File_Names_Offset);
   ...
   begin
      ...
      if Prologue.Unit_Length /= 0 and then
        Off /= Last_Byte_Of_Prologue + 1 then
         ...
         for J in Traceback'Range loop
            declare
               File_Image : constant String :=
                 Offset_To_String (C.Obj, File_Names (J));

               Symbol_Image : constant String :=
                 System.Object_Reader.Decoded_Ada_Name (C.Obj, Symbols (J));
   ...
   begin
      ...
      Seek (C.Obj, Old_Off);
      return Offset_To_String (C.Obj, Off);
   end To_Dir_Name;
   ...
   begin
      ...
      loop
         ...
         Dir_Idx := Read_LEB128 (C.Obj);
         Mod_Time := Read_LEB128 (C.Obj);
         Length := Read_LEB128 (C.Obj);
         exit when J = Code;
      end loop;
