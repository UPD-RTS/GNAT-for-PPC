...
package System.BB.Peripherals is
   ...
   procedure Power_Down_Mode;
   --  Procedure that sets the processor in idle mode.
   --  Should be used for idle loop.
   ...
   Preescaler :  constant Positive := 4;
   Timer_Freq_Hz : constant Positive := Clock_Freq_Hz / Preescaler;
   --  Timer downcount frequency expressed in Hz
   --  It must be noted that a prescaler is used and so the tick is wrong
   --  It should be divided by the Preescaler

   type Timer_Interval is mod 2 ** 24;
   for Timer_Interval'Size use 24;
   ...
   Unused_4                    : constant SBP.Interrupt_Level := 1;
   PCI                         : constant SBP.Interrupt_Level := 1;
   Unused_3                    : constant SBP.Interrupt_Level := 1;
   Unused_2                    : constant SBP.Interrupt_Level := 1;
   DSU                         : constant SBP.Interrupt_Level := 1;
   Unused_1                    : constant SBP.Interrupt_Level := 1;
   Timer_2                     : constant SBP.Interrupt_Level := 1;
   Timer_1                     : constant SBP.Interrupt_Level := 1;
   External_Interrupt_3        : constant SBP.Interrupt_Level := 1;
   External_Interrupt_2        : constant SBP.Interrupt_Level := 1;
   External_Interrupt_1        : constant SBP.Interrupt_Level := 1;
   External_Interrupt_0        : constant SBP.Interrupt_Level := 1;
   UART_1_RX_TX                : constant SBP.Interrupt_Level := 1;
   UART_2_RX_TX                : constant SBP.Interrupt_Level := 1;
   Correctable_Error_In_Memory : constant SBP.Interrupt_Level := 1;
   ...
   type UART_Baudrate is
          range (Clock_Freq_Hz * 10) /
           (((2**12 - 1) * 10 + 5) * 8) .. (Clock_Freq_Hz * 10) /
           (((0 * 10) + 5) * 8);
   --  Range of baudrates supported by the UART
   ...
   pragma Export (C, Initialize_UART, "system_bb_initialize_uart");
   --  Procedure to initialize the UART port

   type UART_Channel is (C1, C2);
   --  Existing channels in the UART

   procedure UART_Send (Char    : Character;
                        Channel : UART_Channel);
   --  Procedure to send Characters to any of the UART channels
