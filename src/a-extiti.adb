...
package body Ada.Execution_Time.Timers is
   ...
   begin
      ...
      --  Now is the execution time of partition.
      ...
      if In_Time > Ada.Real_Time.Time_Span_Zero then
         ...
         if Handler /=  null then
            ...
            if TM.Thread = System.BB.Threads.Queues.Running_Thread then
               ...
               How := Now + To_Time (In_Time);
               System.BB.Threads.Queues.Insert_TM_Alarm
                 (How, Is_First_Alarm);
               Insert_Alarm_If_Needed (Is_First_Alarm, How);
            end if;
         else
         ...
         if Handler /= null then
            ...
            if TM.Thread = System.BB.Threads.Queues.Running_Thread then
               ...
               TM.Thread.Time_Diff :=
                 Now - TM.Thread.Time_Init_Execution;
               --  Insert an alarm if it is needed
               How := System.BB.Time.Time (At_Time);
               System.BB.Threads.Queues.Insert_TM_Alarm
                 (How, Is_First_Alarm);
   ...
   begin
      if TM.Is_Set then
         if TM.Thread = System.BB.Threads.Queues.Running_Thread then
            return To_Time_Span
              (TM.Thread.Time_Remaining -
                 (System.BB.Time.Clock -
                    TM.Thread.Time_Init_Execution));
         else
