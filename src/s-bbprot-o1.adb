...
--        Put_Line ("[SYSTEM.BB.PROTECTION.CONTEXT_SWITCH_NEEDED] First is " &
...
--                    To_Integer (Threads.Queues.Running_Thread)'Img);
      ...
      if B then
         if Threads.Queues.First_Thread.Base_Priority = 2 then
...
--                       Integer'Image (Threads.Queues.Nr_Context_Switches));
      ...
      --  Put_Line ("[SYSTEM.BB.PROTECTION.CONTEXT_SWITCH_NEEDED] Exit");
      return B;
   ...
   begin
      ...
      if Threads.Queues.First_Thread = Threads.Null_Thread_Id then
         ...
         Threads.Queues.Idle_Is_Running := True;
         ...
         Threads.Queues.Insert_In_Queue (Threads.Queues.Running_Thread);
         ...
         while Threads.Queues.First_Thread = Threads.Queues.Running_Thread
           ...
           and then Threads.Queues.Idle_Is_Running
         loop
      ...
      if Context_Switch_Needed then
         ...
         --  Put_Line ("oldTh: " &
         --     To_Integer (Threads.Queues.Running_Thread)'Img);
--           Put_Line ("Switching from thread with priority " &
--                       Integer'Image
--                       (Threads.Queues.Running_Thread.Active_Priority) &
--                       " to thread with priority " &
--                       Integer'Image
--                       (Threads.Queues.First_Thread.Active_Priority));

--           Put_Line ("Context switching");

         if Threads.Queues.First_Thread.Base_Priority = 2 then
            Threads.Queues.Count_Idle := Threads.Queues.Count_Idle + 1;
         end if;
...
--           Put_Line ("Enabling interrupts from kernel mode");
...
--           Put_Line ("Enabling interrupts from user mode");
