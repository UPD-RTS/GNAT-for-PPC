...

/* Helper routine to make a descriptor field.  */
...
build_vms_descriptor (tree type, Mechanism_Type mech, Entity_Id gnat_entity)
{
  ...
  switch (mech)
	...
	= chainon (field_list,
		   make_descriptor_field
		   ("SB_L1", gnat_type_for_size (64, 1), record_type,
		    TREE_CODE (type) == ARRAY_TYPE
		    ? TYPE_MIN_VALUE (TYPE_DOMAIN (type)) : size_zero_node));
      field_list
	= chainon (field_list,
		   make_descriptor_field
		   ("SB_U1", gnat_type_for_size (64, 1), record_type,
		    TREE_CODE (type) == ARRAY_TYPE
		    ? TYPE_MAX_VALUE (TYPE_DOMAIN (type)) : size_zero_node));
      break;
      ...
      for (i = 0; i < ndim; i++)
	{
	  ...
	  field_list
	    = chainon (field_list,
		       make_descriptor_field
		       (fname, gnat_type_for_size (64, 1), record_type,
			TYPE_MIN_VALUE (idx_arr[i])));
...
build_function_stub (tree gnu_subprog, Entity_Id gnat_subprog)
{
  ...
  tree gnu_subprog_param, gnu_stub_param, gnu_param_list, gnu_param;
  ...
  /* Now deal with the unconstrained array case.  In this case the pointer
	...
	(TREE_TYPE (TREE_TYPE (TYPE_FIELDS (ptr))),
	 TREE_TYPE (TREE_TYPE (TYPE_FIELDS (new_ptr))));
      ...
      update_pointer_to
	(TREE_TYPE (TREE_TYPE (TREE_CHAIN (TYPE_FIELDS (ptr)))),
...
gnat_write_global_declarations (void)
{
  ...
  if (types_used_by_cur_var_decl)
	...
	= build_decl (BUILTINS_LOCATION, VAR_DECL, NULL_TREE, void_type_node);
      ...
      for (t = types_used_by_cur_var_decl; t; t = TREE_CHAIN (t))
	types_used_by_var_decl_insert (TREE_PURPOSE (t), dummy_global);

      types_used_by_cur_var_decl = NULL_TREE;
    }
